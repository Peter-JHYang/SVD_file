...
method_one_function <- function(){
  ...
  for( p in c(first_file_number:last_file_number)){
    ...
    split_ten <- split(ten_student, f = ten_student$year) 
    split_neither <- split(neither_ten, f = neither_ten$year) 
    ...
    write.csv(bind_two_ans,paste0("./first_scene/origin_answer/bind_two_ans_",p,".csv"))
    ...
    bind_two_ans <- as.data.frame(bind_two_ans)
    write.csv(bind_two_ans,paste0("./first_scene/svd_answer/svd_two_ans_",p,".csv"))
    ...
    rownames(bind_two_change) <- row_name
    bind_two_change <- as.data.frame(bind_two_change)
    write.csv(bind_two_change,paste0("./first_scene/svd_change/svd_two_change_",p,".csv"))
  }
}
method_one_function()
